export const zhCn = {
  'aside.note': '注意',
  'aside.tip': '提示',
  'aside.caution': '警告',
  'aside.danger': '危险',
  'home.sticky': '置顶',
  'home.goBack': '返回',
  'common.close': '关闭',
  'home.moreArticles': '更多文章',
  'home.readMore': '阅读全文',
  'message.welcome': '欢迎留言',
  'message.welcomeTips': '既然来了就留个脚印吧！',
  'memos.loadMore': '加载更多',
  'memos.loading': '正在加载动态...',
  'memos.empty': '暂时还没有公开动态。',
  'memos.error': '动态加载失败，请稍后再试。',
  'memos.configMissing': 'Memos 配置缺失，暂时无法展示动态。',
  'post.lastUpdated': '最后更新',
  'post.lastUpdatedTip1':'温馨提示：本文最后更新于',
  'post.lastUpdatedTip2':'，部分信息可能因时间推移而不再适用，欢迎反馈。',
  'post.series': '系列文章',
  'post.seriesCountPrefix': '共',
  'post.seriesCountSuffix': '篇',
  'post.seriesCurrent': '当前',
  'post.relatedPosts': '相关文章',
  'remark.open': '展开',
  'sidebar.categories': '分类',
  'sidebar.allCategories': '全部分类',
  'sidebar.uncategorized': '未分类',
  'sidebar.tags': '标签',
  'sidebar.allTags': '全部标签',
  'sidebar.recentArticle': '最近更新',
  'sidebar.recentComments': '最近评论',
  'footer.articleTitle': '本文标题',
  'footer.articleAuthor': '文章作者',
  'footer.releaseTime': '发布时间',
  'footer.originalLink': '原始链接',
  'footer.copyrightOne': '版权声明：本作品采用',
  'footer.copyrightTwo': '「署名-非商业性使用-相同方式共享 4.0 国际」',
  'footer.copyrightThree': '许可协议进行许可',
  'footer.sitemap': '站点地图',
  'footer.busuanziSitePV': '总访问量',
  'footer.busuanziSitePVUnit': '次',
  'footer.busuanziSiteUV': '总访客数',
  'footer.busuanziSiteUVUnit': '人次',
  'feed.publishedIn': '发表于',
  'pagination.total': '共',
  'pagination.unit': '页',
  'title.minutes': '分钟',
  'title.words': '字',
  'title.draft': '草稿',
  'blog.tableOfContent': '文章目录',
  'search.labelOne': '正在查看',
  'search.labelTwo': '下的文章',
  'search.search': '搜索',
  'search.searchLabelOne': '共找到 ',
  'search.searchLabelTwo': ' 篇文章',
  'search.placeholder': '搜索',
  'search.placeholderBasic': '输入标题、摘要、标签或分类关键字',
  'search.placeholderContent': '输入标题、摘要或正文关键字',
  'search.hintBasic': '支持搜索标题、摘要、标签和分类，按 / 快速聚焦',
  'search.hintContent': '支持全文搜索，按 / 快速聚焦',
  'search.loading': '正在加载搜索索引...',
  'search.error': '搜索索引加载失败，请稍后再试。',
  'search.empty': '没有找到相关文章，试试更短的关键字或标签名',
  'feed.previous': '上一条动态',
  'feed.next': '下一条动态',
  'more': '更多'
}
